...
- **Єдиний стиль відображення для всіх трьох runtime.** `BRAVO.Console`
  ...
  «Вимкнено» і «сьогодні не настав час» при цьому різні речі: реставрація
  моделі увімкнена, але має свій день, тому в решту днів лишається
  рядком `SKIPPED`. У Health підсумок друкується з
