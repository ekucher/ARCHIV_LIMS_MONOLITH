...
- **Єдиний стиль відображення для всіх трьох runtime.** `BRAVO.Console`
  ...
  Health і Maintenance переведено на `BRAVO.Console`: заголовок, сім і
  дев'ять етапів відповідно, підсумок із метриками. Вимкнений компонент
  дає рядок `SKIPPED`, а не зникає — інакше нумерація «плавала» б від
  запуску до запуску, і оператор не бачив би різниці між «перевірено» й
  «не перевірялося». У Health підсумок друкується з
