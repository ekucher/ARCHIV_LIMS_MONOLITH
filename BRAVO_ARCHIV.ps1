...
foreach ($runtimeFile in @('BRAVO_COMPATIBILITY.ps1', 'BRAVO_CREDENTIALS.ps1', 'BRAVO_ARCHIV_RUNTIME.ps1')) {
...
$archiveHelpersPath = Join-Path $bravoScriptDirectory 'BRAVO_ARCHIVE_HELPERS.ps1'
if (-not (Test-Path -LiteralPath $archiveHelpersPath -PathType Leaf)) {
    throw "Не знайдено спільні archive helpers: $archiveHelpersPath"
}
. $archiveHelpersPath
...
function Main {
    ...
    if ($backupMonitoring.Enabled -and $backupMonitoring.RunAfterBackup) {
        ...
        try {
                ...
                $healthScriptPath = Join-Path $bravoScriptDirectory 'BRAVO_HEALTH.ps1'
                if (-not (Test-Path -LiteralPath $healthScriptPath -PathType Leaf)) {
                    throw "Не знайдено окремий health-скрипт: $healthScriptPath"
                }
                . $healthScriptPath
                $healthCheckResult = Invoke-BRAVOHealth @healthParameters
