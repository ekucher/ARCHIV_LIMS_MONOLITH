# BRAVO 4.2.13 — архівація, обслуговування та контроль резервних копій
...
При першому запуску Archive/Health/Maintenance мовчки фіксують SHA-256 кожного
наявного інструмента в `Tools\TOOLS_INTEGRITY.json` (trust-on-first-use). На
кожному наступному запуску контрольна сума звіряється: розбіжність лише
породжує попередження в лозі й не блокує роботу — рішення, чи це легітимне
оновлення інструмента, лишається за адміністратором. Щоб прийняти нову
базову лінію після свідомого оновлення `7za.exe`/`WinSCP.*`, видаліть
`TOOLS_INTEGRITY.json`.
