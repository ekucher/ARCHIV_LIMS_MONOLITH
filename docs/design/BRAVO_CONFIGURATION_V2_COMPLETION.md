...
- **C. Default-only / migration-not-required** (вузька категорія без
  жодних конфігураційних файлів) — авторитет: **успішно пройдений**
  no-config preflight (див. «Виняток: journal-free default-only AUTO-
  інсталяція» нижче). Провалений, невизначений чи невідомий результат
  preflight НІКОЛИ не авторизує v2-only рантайм через цей авторитет.
...
- **Сценарій: default-only, потім config-bearing native-v2.**
  ...
  no-config-гейт (Authority C) → входить у native-v2 lifecycle →
  авторитетний native-v2 стан/провенанс зберігається → оператор
  пізніше додає валідний `BRAVO.config` і/або `BRAVO.config.local` →
  конфігурація лишається eligible → журнал міграції все ще не
  вимагається. Нові файли, звісно, усе одно повинні пройти
  non-executing парсер, схему, відхилення невідомих ключів, валідацію
  типів, мердж, деривацію та інваріанти безпеки.
...
- свіжа native-v2 default-only інсталяція + журнал відсутній ⇒
  дозволено (Authority C, до появи конфігураційних файлів);
- default-only native-v2 хост пізніше додає валідний v2-конфіг ⇒
  дозволено без журналу міграції (Authority B успадковує стан з
  Authority C);
