    ...
    if ($lookupEnabled) {
        foreach ($lookupUrl in $lookupUrls) {
            ...
            try {
                ...
                # Недоступність зовнішнього сервісу не перериває обслуговування.
            } finally {
...
function ConvertTo-DiscordNotificationText {
    ...
    # Slack використовує одинарні *, Discord — подвійні ** для жирного тексту.
...
$shouldRestore = $BravoMaintenanceEnabled -and (
    $ForceRestore -or
    ($isRestoreDay -and $isAfterRestoreTime -and -not (Test-Path $MARKER_FILE))
)
