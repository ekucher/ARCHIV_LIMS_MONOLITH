...
# Спільні compatibility та Credential Manager реалізації.
foreach ($runtimeFile in @('BRAVO_COMPATIBILITY.ps1', 'BRAVO_CREDENTIALS.ps1')) {
    $runtimePath = Join-Path $bravoScriptDirectory $runtimeFile
    if (-not (Test-Path -LiteralPath $runtimePath -PathType Leaf)) {
        throw "Не знайдено спільний runtime-файл: $runtimePath"
    }
    . $runtimePath
}
$notificationHelpersPath = Join-Path $bravoScriptDirectory 'BRAVO_NOTIFICATION.ps1'
if (-not (Test-Path -LiteralPath $notificationHelpersPath -PathType Leaf)) {
    throw "Не знайдено спільний notification runtime: $notificationHelpersPath"
}
. $notificationHelpersPath
...
$global:ObjectName = [string]$MaintenanceConfig.General.ObjectName
...
$global:LogLevel = ([string]$MaintenanceConfig.Logging.Level).ToUpperInvariant()
...
if ([string]::IsNullOrWhiteSpace($global:ObjectName) -or
    ...
    $global:LogLevel -notin @("DEBUG", "INFO", "WARNING", "ERROR", "SUCCESS") -or
...
$global:SlackMessageBuffer = New-Object 'System.Collections.Generic.List[string]'
$global:CriticalErrors = $false
$global:CriticalErrorsList = New-Object 'System.Collections.Generic.List[string]'
...
function Restore-FromArchive {
    ...
    if (-not (Test-SevenZipArchiveIntegrity -SevenZipPath $ARC_PATH -ArchivePath $ArchivePath)) {
...
function Test-SevenZipArchiveIntegrity {
    ...
    Write-Log "Перевірка цілісності 7-Zip: $([System.IO.Path]::GetFileName($ArchivePath))" -Level "INFO"
    ...
    if ($testResult.Success) {
        Write-Log "Цілісність архіву підтверджено 7-Zip (код: 0): $ArchivePath" -Level "SUCCESS"
        return $true
    }

    Write-Log "ПОМИЛКА: перевірка цілісності 7-Zip не пройдена (код: $exitCodeText — $($testResult.Description)): $ArchivePath" -Level "ERROR"
    $diagnosticLines = @(
    ...
    if ($diagnosticLines.Count -gt 0) {
        Write-Log "Діагностика 7-Zip test: $($diagnosticLines -join [Environment]::NewLine)" -Level "DEBUG"
    }
    $global:criticalErrorOccurred = $true
    return $false
}
    ...
    foreach ($group in $archiveGroups) {
        ...
        foreach ($archive in @($group.Group)) {
            ...
            try {
                ...
                if (-not (Test-SevenZipArchiveIntegrity -SevenZipPath $ARC_PATH -ArchivePath $archive.FullName)) {
...
function Send-FinalReport {
    ...
    $elapsedTime = (Get-Date) - $global:ScriptStartTime
    $notificationMessage = ""
    $shouldSend = $false
    ...
    if ($global:CriticalErrorsList.Count -gt 0) {
        ...
        $notificationMessage = New-MaintenanceNotificationMessage `
            ...
            -Details @($global:CriticalErrorsList.ToArray()) `
            -LogPath $LOG_FILE
        $shouldSend = $true
    } 
    else {
    ...
    if ($shouldRestore) {
        try {
            ...
            if ($exitCode -ne 0) {
                ...
                Send-SlackAlert -Message $errorMsg -IsCritical
                $global:criticalErrorOccurred = $true
            } elseif (-not (Test-SevenZipArchiveIntegrity -SevenZipPath $ARC_PATH -ArchivePath $beforeArchivePath)) {
