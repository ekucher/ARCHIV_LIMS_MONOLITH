...
# Health використовує спільні реалізації, а не копії з архіватора. Коли
# BRAVO_ARCHIV вже завантажив їх разом зі своїм WinSCP runtime, не перевизначаємо
# функції вдруге.
$requiredRuntime = @(
    [pscustomobject]@{ File = 'BRAVO_COMPATIBILITY.ps1'; Function = 'Enable-BRAVOTls12' },
    [pscustomobject]@{ File = 'BRAVO_CREDENTIALS.ps1'; Function = 'Import-BRAVOInstitutionSettings' },
    [pscustomobject]@{ File = 'BRAVO_ARCHIV_RUNTIME.ps1'; Function = 'Test-BRAVOWinSCPAvailable' }
)
foreach ($runtime in $requiredRuntime) {
    if ($null -ne (Get-Command -Name $runtime.Function -ErrorAction SilentlyContinue)) {
        continue
    }
    $runtimePath = Join-Path $bravoScriptDirectory $runtime.File
